...
**Consequences:** `static-contract.json` REST entries import `moon-discord/rest`; Gateway entries import `moon-discord`. `connect()` and **Gateway send** on the Rest export are configuration errors. Cache later hooks inside **Client** after **Decode** ([Choose the optional cache boundary](https://github.com/Ermianr/moon-discord/issues/11)). Detail: `docs/research/static-core-architecture.md`.
